...
async def _show(event: Message | CallbackQuery, text: str, markup: InlineKeyboardMarkup) -> None:
...
async def cmd_start(message: Message, bot: Bot) -> None:
    ...
    if conn is None:
        me = await bot.me()
        await message.answer(texts.START.format(bot=me.username or "bot"))
    await root(message, bot, owner_id)


@router.message(Command("help"))
async def cmd_help(message: Message) -> None:
    await message.answer(texts.HELP, reply_markup=keyboards.back())
...
async def cmd_away(message: Message, command: CommandObject, bot: Bot) -> None:
    owner_id = message.from_user.id
    if command.args:
        await db.set_setting(owner_id, "away_text", command.args.strip()[: config.MAX_TEXT_LEN])
...
async def on_menu(call: CallbackQuery, bot: Bot, state: FSMContext) -> None:
    ...
    if screen == "root":
        ...
        archive = await db.archive_size(owner_id)
        await _show(call, texts.spy_card(st, archive), keyboards.spy(st))
    elif screen == "chats":
        ...
        await _show(call, texts.HELP, keyboards.back())
    await call.answer()


#: Что вообще можно переключать кнопкой. Проверять по SETTING_FIELDS
#: мало: там же лежат тексты и паузы, и «переключение» превратило бы
#: текст приветствия в единицу.
TOGGLEABLE = ("active", "away", "greet", "fallback", "spy", "spy_edits")
...
async def on_toggle(call: CallbackQuery, bot: Bot) -> None:
    ...
    if field not in TOGGLEABLE:
        ...
        return
    now_on = await db.toggle_setting(owner_id, field)
    st = await db.settings_of(owner_id)
    ...
    await call.answer(hint[0] if now_on else hint[1])
...
async def on_keep(call: CallbackQuery) -> None:
    ...
    st = await db.settings_of(owner_id)
    await _show(call, texts.spy_card(st, await db.archive_size(owner_id)), keyboards.spy(st))
...
async def on_clear(call: CallbackQuery) -> None:
    ...
    st = await db.settings_of(owner_id)
    await _show(call, texts.spy_card(st, 0), keyboards.spy(st))
...
async def on_ignore(call: CallbackQuery) -> None:
    ...
    # Кнопка живёт в двух местах. На экране «Чаты» (ig:) перерисовываем
    # список, под карточкой удалённого сообщения (ign:) перерисовывать
    # нечего — там текст, а не меню, поэтому только всплывашка.
    if prefix == "ig":
        rows = await db.recent_chats(owner_id, config.CHATS_PAGE)
        await _show(call, texts.chats_card(rows), keyboards.chats(rows))
    await call.answer(
...
async def start_rule(event: Message | CallbackQuery, state: FSMContext, owner_id: int) -> None:
...
async def on_samples(call: CallbackQuery) -> None:
    ...
    rules = await db.rules_of(owner_id)
    await _show(call, texts.rules_list(rules), keyboards.rules(rules))
    await call.answer(texts.SAMPLES_ADDED.format(n=len(config.SAMPLE_RULES)), show_alert=True)
...
async def on_rule(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if action == "t":
        ...
        rule = await db.rule_of(owner_id, rule_id)
        await _show(call, texts.rule_card(rule), keyboards.rule(rule_id, rule["enabled"]))
...
async def on_edit_text(call: CallbackQuery, state: FSMContext) -> None:
    field = call.data.split(":", 1)[1]
    if field not in ("away_text", "greet_text", "fallback_text"):
        await call.answer()
        return
    st = await db.settings_of(call.from_user.id)
...
async def got_triggers(message: Message, state: FSMContext) -> None:
    ...
    if len(parts) > config.MAX_TRIGGERS:
        await message.answer(f"Слишком много: максимум {config.MAX_TRIGGERS} через <code>|</code>.")
        return
...
async def got_rule_reply(message: Message, state: FSMContext) -> None:
    ...
    if rule is None:
        ...
        return
    await message.answer(texts.rule_card(rule), reply_markup=keyboards.rule(rule_id, rule["enabled"]))
...
async def got_text_field(message: Message, state: FSMContext) -> None:
    data = await state.get_data()
    field = data["field"]
    await db.set_setting(message.from_user.id, field, message.text.strip()[: config.MAX_TEXT_LEN])
    await state.clear()
...
@router.message(StateFilter(Form.triggers, Form.reply, Form.text_field, Form.rule_reply))
